...
from agents import parser, fetcher, cleaner, analyzer, reporter , profiler , analysis_strategy , cleaning_strategy, feature_engineering
...
def run_pipeline(user_input):
    ...
    logger.info(f"Cleaning plan complete: {clean_plan}")
    memory.set("cleaning_plan", clean_plan)

    analysis_plan = analysis_strategy.get_analysis_plan(profile)
    logger.info(f"Analysis plan complete: {analysis_plan}")
    ...
    df_featured = feature_engineering.engineer_features(df_clean, analysis_plan)
